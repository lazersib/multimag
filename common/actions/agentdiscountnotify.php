	...
	public function run() {
		...
		while($agent_info = $res->fetch_assoc()) {
			...
			if($sum<$cur_price_sum) {
				...
				$mail_text .= "Для сохранения существующей специальной цены \"$cur_price_sum_name\" Вам необходимо совершить в текущем периоде дополнительных поупок на сумму не менее $s рублей.\n";
			}
			else if($next_price_sum && $cur_price_sum) {
				$s = number_format ($next_price_sum-$sum, 2, '.' , ' ');
				$mail_text .= "Для получения специальной цены \"$next_price_name\" Вам необходимо совершить в текущем периоде дополнительных поупок на сумму не менее $s рублей.\n";
			}
			else $no_notify = 1;
