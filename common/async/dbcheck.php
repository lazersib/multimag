...
class dbcheck extends \AsyncWorker {
    ...
    /// Уведомление в случае несовпадения версий базы и кода
    ...
    ///Сброс остатков
    protected function zeroCounters() {
        global $db;
    ...
    // Поиск проведённых документов с пометкой на удаление (такое сочетание - ошибка)
    protected function checkCorrectDelMark() {
        global $db;
    ...
    /// Расчет дат первой покупки
    protected function calcBuyDate() {
        global $db;
    ...
    /// Расчёт ликвидности
    protected function calcLiquidity() {
        global $db;
    ...
    protected function checkStoreOnPosMove() {
        global $db;
    ...
    protected function deleteMarkedDocs() {
        global $db;
        $this->SetStatusText("Удаление помеченных на удаление...\n");
        $tim_minus = time() - 60 * 60 * 24 * \cfg::get('auto', 'doc_del_days');
    ...
    /// Проверка отсутствия поступлений по нулевым ценам
    protected function checkIncomingZeroPrices() {
        global $db;
    ...
    /// Перепроводка докуменов
    protected function reApplyDoc() {
        global $db;
        ...
        if ($i) {
            ...
            echo"Ошибки последовательности документов не найдены!\n";
        }
    }
    ...
    function run() {
        ...
        $this->checkRevision();
        $this->zeroCounters();
        $this->checkCorrectDelMark();
        $this->calcBuyDate();
        $this->calcLiquidity();
        $this->checkStoreOnPosMove();
        $this->deleteMarkedDocs();
        $this->checkIncomingZeroPrices();
        $this->reApplyDoc();
        ...
        if ($this->mail_text) {
            try {
                ...
                mailto(\cfg::get('site', 'doc_adm_email'), "DB check report", $mail_text);
