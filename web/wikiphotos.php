...
if(!$n)
{
        $res=mysql_query("SELECT `id` FROM `wikiphoto`");
        $row=mysql_num_rows($res);
        $rnd=rand(0,$row-1);
        $n=mysql_result($res,$rnd,0);
}
...
WHERE `wikiphoto`.`id`='$n' ");
if($nxt=mysql_fetch_row($res))
{
	$imagefile="{$CONFIG['site']['var_data_fs']}/wikiphoto/$n.jpg";
	settype($cc,"array");
	$cc[0]=$CONFIG['site']['name'];
	$cc[1]=$nxt[2];
	//$cc[2]=translit($nxt[2]);
}
else {echo"err($n): ".mysql_error(); exit();};


$rs=0;


$cname="{$CONFIG['site']['var_data_fs']}/cache/wikiphoto/$n-$x-$y-$q.jpg";
$icname="{$CONFIG['site']['var_data_web']}/cache/wikiphoto/$n-$x-$y-$q.jpg";
