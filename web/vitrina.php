...
class Vitrina {
	...
	function __construct() {
		...
		if (@ $_SESSION['uid'])
			...
			$res = mysql_query("SELECT `id` FROM `doc_cost` WHERE `vid`='1'");
		if (mysql_errno())
			throw new MysqlException('Не удалось выбрать цену для пользователя');
	...
	protected function TopGroup() {
		...
		if ($CONFIG['site']['vitrina_glstyle'] == 'item')
			$this->GroupList_ItemStyle(0);
		else
			$this->GroupList_ImageStyle(0);
	}
	...
	protected function ProductList($group, $page) {
		...
		if (isset ($_REQUEST['order']))
			...
			if (isset ($_SESSION['vitrina_order']))
		...
		switch ($order) {
			case 'n' :
				...
				break;
			case 'nd' :
				$sql_order = '`doc_base`.`name` DESC';
				break;
			case 'vc' :
				$sql_order = '`doc_base`.`vc`';
				break;
			case 'vcd' :
				$sql_order = '`doc_base`.`vc` DESC';
				break;
			case 'c' :
				$sql_order = '`doc_base`.`cost`';
				break;
			case 'cd' :
				$sql_order = '`doc_base`.`cost` DESC';
				break;
			case 's' :
				$sql_order = '`count`';
				break;
		...
		if (isset ($_REQUEST['view']))
			...
			if (isset ($_SESSION['vitrina_view']))
		...
		if ($view != 'i' && $view != 'l' && $view != 't') {
			if ($CONFIG['site']['vitrina_plstyle'] == 'imagelist')
				...
				if ($CONFIG['site']['vitrina_plstyle'] == 'extable')
					$view = 't';
				else
					$view = 'l';
		}
		...
		if ($rows) {
			...
			if (($lim < $rows) && $page)
				mysql_data_seek($res, $lim * ($page -1));
			if ($view == 'i')
	...
	protected function OrderAndViewBar($group, $page, $order, $view) {
		...
		if ($view == 'i')
			$tmpl->AddText("<span class='selected'>Картинками</span> ");
		else
			$tmpl->AddText("<span><a href='" . $this->GetGroupLink($group, $page, 'view=i') . "'>Картинками</a></span> ");
		if ($view == 't')
			$tmpl->AddText("<span class='selected'>Таблицей</span> ");
		else
			$tmpl->AddText("<span><a href='" . $this->GetGroupLink($group, $page, 'view=t') . "'>Таблицей</a></span> ");
		if ($view == 'l')
			$tmpl->AddText("<span class='selected'>Списком</span> ");
		else
			$tmpl->AddText("<span><a href='" . $this->GetGroupLink($group, $page, 'view=l') . "'>Списком</a></span> ");
		if (@ $_SESSION['vit_photo_only'])
			$tmpl->AddText("<span class='selected'><a class='down'  href='" . $this->GetGroupLink($group, $page, 'op=0') . "'>Только с фото</a></span> ");
		else
			$tmpl->AddText("<span><a href='" . $this->GetGroupLink($group, $page, 'op=1') . "'>Только с фото</a></span> ");
		$tmpl->AddText("</div>");
		$tmpl->AddText("<div class='viewbar'>Сортировать по: ");
		if ($order == 'n')
			$tmpl->AddText("<span class='selected'><a href='" . $this->GetGroupLink($group, $page, 'order=nd') . "'>Названию</a></span> ");
		else
			if ($order == 'nd')
				$tmpl->AddText("<span class='selected'><a class='down' href='" . $this->GetGroupLink($group, $page, 'order=n') . "'>Названию</a></span> ");
			else
				$tmpl->AddText("<span><a href='" . $this->GetGroupLink($group, $page, 'order=n') . "'>Названию</a></span> ");

		if ($order == 'vc')
			$tmpl->AddText("<span class='selected'><a href='" . $this->GetGroupLink($group, $page, 'order=vcd') . "'>Коду</a></span> ");
		else
			if ($order == 'vcd')
				$tmpl->AddText("<span class='selected'><a href='" . $this->GetGroupLink($group, $page, 'order=vc') . "'>Коду</a></span> ");
			else
				$tmpl->AddText("<span><a class='down' href='" . $this->GetGroupLink($group, $page, 'order=vc') . "'>Коду</a></span> ");

		if ($order == 'c')
			$tmpl->AddText("<span class='selected'><a href='" . $this->GetGroupLink($group, $page, 'order=cd') . "'>Цене</a></span> ");
		else
			if ($order == 'cd')
				$tmpl->AddText("<span class='selected'><a class='down' href='" . $this->GetGroupLink($group, $page, 'order=c') . "'>Цене</a></span> ");
			else
				$tmpl->AddText("<span><a href='" . $this->GetGroupLink($group, $page, 'order=c') . "'>Цене</a></span> ");

		if ($order == 's')
			$tmpl->AddText("<span class='selected'><a href='" . $this->GetGroupLink($group, $page, 'order=sd') . "'>Наличию</a></span> ");
		else
			if ($order == 'sd')
				$tmpl->AddText("<span class='selected'><a class='down' href='" . $this->GetGroupLink($group, $page, 'order=s') . "'>Наличию</a></span> ");
			else
				$tmpl->AddText("<span><a href='" . $this->GetGroupLink($group, $page, 'order=s') . "'>Наличию</a></span> ");
	...
	protected function ProductCard($product) {
		...
		if ($nxt = mysql_fetch_array($res)) {
			if ($nxt['title_tag'])
				...
				$title = "{$nxt['group_printname']} {$nxt['name']}, цены и характеристики, купить";
			$tmpl->SetTitle($title);
			...
			if ($nxt['meta_description'])
				...
				$d[2] = array (
					' в интернет-магазине ' . $CONFIG['site']['display_name'] . ' по '
				);
				$d[3] = array (
					'доступной',
					'отличной',
			...
			if ($nxt['img_id']) {
				...
				while ($img_data = mysql_fetch_assoc($res)) {
					...
					if (mysql_num_rows($res) > 1)
						$img_mini .= "<a href='" .
						$midiimg->GetURI() . "' onclick=\"return setPhoto({$img_data['img_id']});\"><img src='" . $miniimg->GetURI() . "' alt='{$img_data['name']}'></a>";
			...
			if ($nxt[14])
				$tmpl->AddText("<tr><td class='field'>Тип:<td>$nxt[14]");
			...
			if ($nxt[11])
				$tmpl->AddText("<tr><td class='field'>Наличие: <td><b>ЕСТЬ</b><br>");
			else
				$tmpl->AddText("<tr><td class='field'>Наличие:<td>Под заказ<br>");
			if ($nxt[6])
				$tmpl->AddText("<tr><td class='field'>Внутренний диаметр: <td>$nxt[6] мм.<br>");
			if ($nxt[7])
				$tmpl->AddText("<tr><td class='field'>Внешний диаметр: <td>$nxt[7] мм.<br>");
			if ($nxt[8])
				$tmpl->AddText("<tr><td class='field'>Высота: <td>$nxt[8] мм.<br>");
			if ($nxt[9])
				$tmpl->AddText("<tr><td class='field'>Масса: <td>$nxt[9] кг.<br>");
			if ($nxt[10])
				$tmpl->AddText("<tr><td class='field'>Аналог: <td>$nxt[10]<br>");
			if ($nxt[5])
			...
			$d[] = array (
				$nxt['group_printname'] . ' ' . $nxt['name'] . ' ' . $nxt['proizv'] . ' по '
			);
			$d[] = array (
				'доступной',
				'отличной',
			...
			foreach ($CONFIG['payments']['types'] as $type => $val) {
				...
				switch ($type) {
					case 'cash' :
						...
						break;
					case 'bank' :
						$s = "<label><input type='radio' name='pay_type' value='$type'$checked>Безналичный банкосвкий перевод.
	...
	protected function MakeBuy() {
		...
		if ($_SESSION['basket']['cnt']) {
			...
			if (mysql_errno())
				throw new MysqlException("Не удалось определить банк");
			if (mysql_num_rows($res) < 1)
	...
	protected function GetVitPath($group_id) {
		...
		$nxt = mysql_fetch_row($res);
		if (!$nxt)
			return "<a href='/vitrina.php'>Витрина</a>";
		...
		if ($CONFIG['site']['vitrina_pcnt'] == 1) {
			if ($count <= 0) {
				...
				if ($count <= $CONFIG['site']['vitrina_pcnt_limit'][0])
					...
					if ($count <= $CONFIG['site']['vitrina_pcnt_limit'][1])
						return '**';
					else
						if ($count <= $CONFIG['site']['vitrina_pcnt_limit'][2])
							return '***';
						else
							return '****';
		} else
...
try {
	...
	if (file_exists($CONFIG['site']['location'] . '/skins/' . $CONFIG['site']['skin'] . '/vitrina.tpl.php'))
		include_once ($CONFIG['site']['location'] .
		'/skins/' . $CONFIG['site']['skin'] . '/vitrina.tpl.php');
	if (!isset ($vitrina))
		$vitrina = new Vitrina();
