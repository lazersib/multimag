...
class createPDF {
    ...
    function createPDF($_html,$_title,$_articleurl,$_author,$_date) {
        ...
        $this->to='cp1251';               // output encoding
...
class PDF extends FPDF
{
    ...
    function PDF($orientation='P',$unit='mm',$format='A4',$_title,$_url,$_debug=false)
    {
        ...
        $this->HREF='';
        $this->PRE=false;
        $this->AddFont('Times','','arial.php');
        $this->AddFont('Courier','','arial.php');
	...
	function Cell($w, $h=0, $txt='', $border=0, $ln=0, $align='', $fill=false, $link='')
	{
		...
		if($txt!='')
		{
			...
			{
				//Set word spacing
				if($this->GetStringWidth($txt)>$w*0.8)
				{
					$wmax=($w-2*$this->cMargin);
					$this->ws=($wmax-$this->GetStringWidth($txt))/(substr_count($txt,' ')+0.001);
            ...
            if (!$skip) {
                ...
                if($i%2==0)
                {
                    ...
                    if($this->PRE)
                        ...
                        $e=str_replace("\r","",$e);
                    //Text
                    $align=$this->center?'C':'J';
                    if(strlen(trim(stripslashes(txtentities($e))))>0)
                    	$this->MultiCell(0, $this->str_w*0.93, stripslashes(txtentities($e)), 0, $align);
    ...
    function OpenTag($tag,$attr)
    {
        ...
        switch($tag){
            ...
            case 'LI':
                //$this->Ln(6);
                if($this->ol_id==1)	$this->SetFontSize(14);
                ...
                if($this->ol_id)
                {
                	...
                	$str='';
                	foreach($this->li_id as $li_id)	$str.=$li_id.'.';
                	$str.='  ';
                	$this->SetX($this->lMargin-$this->GetStringWidth($str));
                	$this->Write($this->str_w,$str);
