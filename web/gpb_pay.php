...
try {
	...
	WHERE `doc_list`.`id`='$merchant_trx' AND `doc_list`.`type`='24'");
    if (!$res->num_rows) {
        throw new Exception("Банк-приход не найден");
    }
    ...
    if ($result_code == 1) {
        if ($amount != round($b_info['sum'] * 100)) {
            throw new Exception("Неверная сумма платежа ($amount != {$b_info['sum']})");
        }
