...
if($mode=='')
{
	...
	$res=mysql_query("SELECT `id`, `text` FROM `questions` ORDER BY `id`");
	while($nxt=mysql_fetch_row($res))
	{
		...
		{
			$r=mysql_query("SELECT `id` FROM `question_answ` WHERE `q_id`='$nxt[0]' AND `answer` LIKE '%|$nx[0]|%'");
			$cnt=mysql_num_rows($r);
			$arr[$nx[0]]['name']=$nx[1];
			$arr[$nx[0]]['cnt']=$cnt;
			$max+=$cnt;	
