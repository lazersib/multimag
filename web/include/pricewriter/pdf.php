...
class PriceWriterPDF extends BasePriceWriter
{
	var $pdf;
...
//		$pdf->Cell(0,5,$str,0,1,'C');
		...
		if(is_array($this->view_groups))	{
			...
			$this->str = 'Прайс содержит неполный список позиций, в соответствии с выбранными критериями при его загрузке с сайта.';
			$this->str = iconv('UTF-8', 'windows-1251', $str);
	...
	function write()	{
		...
		if($this->view_proizv) $proizv='`doc_base`.`proizv`';
		else 			$proizv="''";
