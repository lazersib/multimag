...
class Report_Profitability extends BaseGSReport {
    ...
    function getName($short = 0) {
        if ($short) {
            return "По рентабельности и прибыли";
        } else {
            return "Отчёт по рентабельности и прибыли";
        }
    }
...
//		settype($date_to, 'int');
        ...
        $res = $db->query("SELECT `doc_list_pos`.`cnt`, `doc_list_pos`.`cost`, `doc_list`.`type`, `doc_list_pos`.`page`,
                `doc_dopdata`.`value` AS `ret_flag`, `doc_list`.`date`, `doc_list`.`firm_id`
            FROM `doc_list_pos`
        ...
        while ($nxt = $res->fetch_assoc()) {
            ...
            $cnt = round($cnt, 3);
            if ($cnt < 0) {
                return array(0xFFFFBADF00D, 0, 0);
            } // Невозможно расчитать прибыль, если остатки уходили в минус
        }
        if ($out_cnt) {
...
//		settype($date_to, 'int');
        ...
        $res = $db->query("SELECT `doc_list_pos`.`cnt`, `doc_list_pos`.`cost`, `doc_list`.`type`, `doc_list_pos`.`page`, `doc_dopdata`.`value`
                , `doc_list`.`date`, `doc_list`.`firm_id`
            FROM `doc_list_pos`
            INNER JOIN `doc_list` ON `doc_list`.`id`=`doc_list_pos`.`doc` AND (`doc_list`.`type`<='2' OR `doc_list`.`type`='17')
            LEFT JOIN `doc_dopdata` ON `doc_dopdata`.`doc`=`doc_list_pos`.`doc` AND `doc_dopdata`.`param`='return'
            WHERE `doc_list_pos`.`tovar`='$pos_id' AND `doc_list`.`ok`>'0' AND `doc_list`.`date`>='$date_from' AND `doc_list`.`date`<='$date_to'
