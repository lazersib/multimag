...
class Report_OstatkiNaDatu extends BaseGSReport {
    ...
    function Make($engine) {
        ...
        while ($group_line = $res_group->fetch_assoc()) {
            ...
            while ($nxt = $res->fetch_row()) {
                ...
                if ($CONFIG['poseditor']['vc']) {
                    ...
                    $a = array($nxt[0], $nxt[1], $count_p, $cost_p, $bsum_p);
                }
                $this->tableRow($a);
            }
        }
        ...
        if (!$zeroflag) {
            ...
            $this->tableSpannedRow(array($col_count), array("Общая масса склада: невозможно определить из-за отрицательных остатков"));
        }
        $this->tableAltStyle(false);
        $this->tableEnd();
        $this->output();
