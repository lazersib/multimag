...
//
/// Отчёт по движению товара без цен
class Report_Move_NoCost extends BaseGSReport {
	...
	function Form() {
		...
		while ($nxt = $res->fetch_row())
			$tmpl->addContent("<option value='$nxt[0]'>$nxt[1]</option>");
	...
	function dividedOutPos($pos_id, $vc, $name, $dt_f, $dt_t) {
		...
		if ($this->w_docs) {
			...
			$this->tableSpannedRow(array($this->col_cnt - 1, 1), array('На начало периода:', $start_cnt));
			$this->tableAltStyle();
			$this->tableSpannedRow(array($this->col_cnt), array('Приходы'));
			$this->tableAltStyle(false);
		}
		$rres = $db->query("SELECT `doc_list`.`id`, `doc_list`.`type`, `doc_list`.`agent`, `doc_agent`.`name` AS `agent_name`, `doc_list_pos`.`cnt`, `doc_list_pos`.`cost`, `ns`.`value` AS `na_sklad`, `doc_sklady`.`name` AS `sklad_name`, `doc_types`.`name` AS `doc_name`, `doc_list`.`date`, CONCAT(`doc_list`.`altnum`, `doc_list`.`subtype`) AS `snum`
		FROM `doc_list_pos`
		...
		if ($this->w_docs)
			$this->tableRow(array('', 'Всего приход:', '', $prix_cnt));
		$r_cnt = 0;
		...
		$pres = $db->query("SELECT `doc_list`.`id`, `doc_list`.`type`, `doc_list`.`agent`, `doc_agent`.`name` AS `agent_name`, `doc_list_pos`.`cnt`, `doc_list_pos`.`cost`, `ns`.`value` AS `na_sklad`, `doc_sklady`.`name` AS `sklad_name`, `doc_types`.`name` AS `doc_name`, `doc_list`.`date`, CONCAT(`doc_list`.`altnum`, `doc_list`.`subtype`) AS `snum`
		FROM `doc_list_pos`
		INNER JOIN `doc_list` ON `doc_list`.`id`=`doc_list_pos`.`doc`
		INNER JOIN `doc_types` ON `doc_types`.`id`=`doc_list`.`type`
		LEFT JOIN `doc_dopdata` AS `ns` ON `ns`.`doc`=`doc_list_pos`.`doc` AND `ns`.`param`='na_sklad'
		...
		if ($this->w_docs)
			$this->tableRow(array('', 'По реализациям:', '', $realiz_cnt, ''));
		$r_cnt+=$realiz_cnt;
		if ($this->w_docs) {
			$this->tableAltStyle();
		...
		$vres = $db->query("SELECT `doc_list`.`id`, `doc_list`.`type`, `doc_list`.`agent`, `doc_agent`.`name` AS `agent_name`, `doc_list_pos`.`cnt`, `doc_list_pos`.`cost`, `ns`.`value` AS `na_sklad`, `doc_sklady`.`name` AS `sklad_name`, `doc_types`.`name` AS `doc_name`, `doc_list`.`date`, CONCAT(`doc_list`.`altnum`, `doc_list`.`subtype`) AS `snum`
		FROM `doc_list_pos`
		INNER JOIN `doc_list` ON `doc_list`.`id`=`doc_list_pos`.`doc`
		INNER JOIN `doc_types` ON `doc_types`.`id`=`doc_list`.`type`
		LEFT JOIN `doc_dopdata` AS `ns` ON `ns`.`doc`=`doc_list_pos`.`doc` AND `ns`.`param`='na_sklad'
		...
		if ($this->w_docs)
			$this->tableRow(array('', 'По перемещениям:', '', $perem_cnt));
		$r_cnt+=$perem_cnt;
		if ($this->w_docs) {
			$this->tableAltStyle();
		...
		$zres = $db->query("SELECT `doc_list`.`id`, `doc_list`.`type`, `doc_list`.`agent`, `doc_agent`.`name` AS `agent_name`, `doc_list_pos`.`cnt`, `doc_list_pos`.`cost`, `ns`.`value` AS `na_sklad`, `doc_sklady`.`name` AS `sklad_name`, `doc_types`.`name` AS `doc_name`, `doc_list`.`date`, CONCAT(`doc_list`.`altnum`, `doc_list`.`subtype`) AS `snum`
		FROM `doc_list_pos`
		INNER JOIN `doc_list` ON `doc_list`.`id`=`doc_list_pos`.`doc`
		INNER JOIN `doc_types` ON `doc_types`.`id`=`doc_list`.`type`
		LEFT JOIN `doc_dopdata` AS `ns` ON `ns`.`doc`=`doc_list_pos`.`doc` AND `ns`.`param`='na_sklad'
	...
	function serialOutPos($pos_id, $vc, $name, $dt_f, $dt_t) {
		...
		$res = $db->query("SELECT `doc_list`.`id`, `doc_list`.`type`, `doc_list`.`sklad`, `doc_list_pos`.`page`,
	...
	function outPos($pos_id, $vc, $name, $dt_f, $dt_t) {
		if ($this->div_dt || !$this->w_docs)
			$this->dividedOutPos($pos_id, $vc, $name, $dt_f, $dt_t);
		else
			$this->serialOutPos($pos_id, $vc, $name, $dt_f, $dt_t);
	}
	...
	function Make($engine) {
		...
		if ($sel_type == 'pos') {
			$pos_id = rcv('pos_id');
			$res = $db->query("SELECT `vc`, `name` FROM `doc_base` WHERE `id`='$pos_id'");
			if ($res->num_rows == 0)	throw new Exception("Товар не выбран!");
			$tov_data = $res->fetch_row();
			$this->outPos($pos_id, $tov_data[0], $tov_data[1], $dt_f, $dt_t);
		}
		else if ($sel_type == 'all') {
			$res = $db->query("SELECT `id`, `vc`, CONCAT(`doc_base`.`name`, ' - ', `doc_base`.`proizv`) AS `name` FROM `doc_base` ORDER BY $order");
			while ($nxt = $res->fetch_row())
			...
			while ($group_line = $res_group->fetch_assoc()) {
				if (is_array($g) && !in_array($group_line['id'], $g))	continue;
