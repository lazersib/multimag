...
class Report_Ved_Agentov
{
	...
	{
		global $tmpl;
		$dt_f=strtotime(@$_REQUEST['dt_f']);
		$dt_t=strtotime(@$_REQUEST['dt_t']." 23:59:59");
		$sel_type=@$_REQUEST['sel_type'];
		$agent=intval(@$_REQUEST['agent']);
		$ag_sel_group=intval(@$_REQUEST['ag_sel_group']);
		$debt=intval(@$_REQUEST['debt']);
		$detail_ag=intval(@$_REQUEST['detail_ag']);
		$detail_doc=intval(@$_REQUEST['detail_doc']);
		$tmpl->AddText("<h1 id='page-title'>Ведомость по агентам</h1>
		<table class='list' width='100%'>
		<tr><th rowspan='2'>Дата</th><th rowspan='2'>Документ</th><th rowspan='2'>Нач.долг</th><th rowspan='2'>Увеличение долга</th><th rowspan='2'>Уменьшение долга</th><th colspan='2'>На конец периода</th></tr>
		...
		WHERE `ok`>'0' AND `mark_del`='0' AND `date`>='$dt_f' AND `date`<='$dt_t'
		...
		while($line=mysql_fetch_assoc($res))
		{
			...
			$startdebt='';
			$end_nd=$end_cd='';
			if($dolg>0)		$end_cd=$dolg;
			else if($dolg<0)	$end_nd=-$dolg;
			
			if($sel_type!='pos' && $detail_ag==0)	$info=" ({$line['agent_name']})";
			
			$tmpl->AddText("<tr><td>$date_p</td><td><a href='/doc.php?mode=body&amp;id={$line['doc_id']}'>{$line['doc_typename']} N{$line['altnum']}{$line['subtype']} / {$line['doc_id']}</a>{$info}</td><td>$startdebt</td>
