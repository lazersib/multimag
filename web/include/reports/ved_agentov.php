...
class Report_Ved_Agentov {
	...
	protected function makeBlock($head, $agent_where = '') {
		global $db;
		$res = $db->query("SELECT `doc_list`.`id` AS `doc_id`, `doc_list`.`altnum` ,`doc_list`.`subtype`, `doc_list`.`type` AS `doc_type`, `doc_list`.`sum` AS `doc_sum`, `doc_list`.`date`, `doc_types`.`name` AS `doc_typename`, `doc_agent`.`name` AS `agent_name`
		FROM `doc_list`
