...
class Report_Incost extends BaseGSReport {
    ...
    function getName($short = 0) {
        if ($short) {
            return "По себестоимости проданных товаров";
        } else {
            return "Отчёт по себестоимости проданных товаров";
        }
    }
    ...
    function Make($engine) {
        ...
        $res = $db->query("SELECT `doc_list`.`id`, `doc_list`.`date`, `doc_list`.`altnum`, `doc_list`.`firm_id`"
            . "FROM `doc_list` "
        ...
        while ($doc_info = $res->fetch_assoc()) {
            if(!\acl::testAccess([ 'firm.global', 'firm.'.$doc_info['firm_id']], \acl::VIEW)) {
                continue;
            }
            switch ($grp) {
                case 'd':
