...
class Report_Sales extends BaseGSReport {
    ...
    function Make($engine) {
        ...
        if (!$this->w_docs) {
            ...
            $headers = array('ID', 'Код', 'Наименование', 'Код поставщика', 'Посл.приход', 'Баз.цена', 'Нач.кол-во', 'Приход', 'Реализ.', 'Перем.', 'Сборка', 'Итог');
        ...
        $sql_fields = "`doc_base`.`id`, `doc_base`.`vc`, CONCAT(`doc_base`.`name`, ' - ', `doc_base`.`proizv`) AS `name`"
                . ", `doc_base`.`cost` AS `base_price`, `vals`.`value` AS `provider_code`";
