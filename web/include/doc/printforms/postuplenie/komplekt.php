...
class komplekt extends \doc\printforms\iPrintFormInvoicePdf {
    protected $form_basesum;
    
    public function getName() {
        return "Накладная на склад";
    }
    ...
    protected function addFormHeaderBlock() {
        ...
        $text = "Накладная на склад N  {$doc_data['altnum']}{$doc_data['subtype']} ($doc_id) от " . date("d.m.Y", $doc_data['date']);
        $this->addHeader($text);  
    ...
    protected function addNomenclatureTableBlock() {
        ...
        $th_widths = array_merge($th_widths, array(10, 14, 12, 14, 20));
        $th_texts = array_merge($th_texts, array('Ед.', 'Кол-во', 'Склад', 'Резерв', 'Место'));
        $tbody_aligns = array_merge($tbody_aligns, array('C', 'R', 'R', 'R', 'R'));
        $this->addTableHeader($th_widths, $th_texts, $tbody_aligns);        
        $font_sizes = array_merge($font_sizes, array(8, 10, 8, 8, 8, 8, 8));
    ...
    protected function addSignBlock() {    
        ...
        $this->addSignLine($text);
        $text = "На склад положил: ___________________________________ ( $klad_name )";
        $this->addSignLine($text);
    }
