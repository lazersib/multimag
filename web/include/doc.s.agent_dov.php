...
class doc_s_Agent_dov {
    ...
    function View() {
        global $tmpl;
        doc_menu();
        \acl::accessGuard('directory.attorney', \acl::VIEW);
    ...
    function Service() {
        ...
        if ($opt == 'pl') {
            ...
            if ($res->num_rows) {
                ...
                $tmpl->addContent("<b>Искомая комбинация " . html_out($s) . " не найдена!</b>");
            }
        } else {
            $tmpl->msg("Неверный режим!");
        }
    }
...
/// Редактирование доверенного лица
    ...
    function ESave() {
        ...
        if ($param == '') {
            ...
            if ($pos) {
                \acl::accessGuard('directory.attorney', \acl::UPDATE);
                ...
                \acl::accessGuard('directory.attorney', \acl::CREATE);
