...
function Run($mode)
{
	...
	else
	{
		...
		else
		{
			...
			{
				$tmpl->AddText("<tr><td colspan='6' style='font-size: 1.15em; color: #000; background-color: #beb;'>$nxt[1]</td></tr>");
...
{
	global $tmpl;
	$res=mysql_query("SELECT `doc_list`.`id`, `doc_list`.`type`, `doc_list`.`sum`, `doc_types`.`name` AS `doc_name`, `doc_list`.`date`, `doc_list`.`altnum`, `doc_list`.`subtype`
	FROM `doc_list`
	INNER JOIN `doc_types` ON `doc_types`.`id`=`doc_list`.`type`
	WHERE `doc_list`.`date`>='$d_from' AND `doc_list`.`date`<='$d_to' AND `doc_list`.`ok`>'0' AND `doc_list`.`mark_del`='0' AND `doc_list`.`agent`='$agent'");
	$sum=0;
	while($nxt=mysql_fetch_row($res))
	{
		$prix=$rasx='';
		switch($nxt[1])
		{
			...
			case 7:	$rasx=$nxt[2];
				...
				break;
			case 18:$sum-=$nxt[2];
				if($nxt[2]>0)	$rasx=$nxt[2];
				else		$prix=$nxt[2];
		}
		$date_p=date("Y-m-d", $nxt[4])."<br><small>".date("H:i:s",$nxt[4])."</small>";
		$tmpl->AddText("<tr class='pointer'><td align='right'><a href='/doc.php?mode=body&doc=$nxt[0]'>$nxt[0]</a></td><td align='right'>$date_p</td><td>$nxt[3] N{$nxt[5]}{$nxt[6]}</td> <td>$prix</td><td>$rasx</td><td>$sum</td></tr>");
	}
