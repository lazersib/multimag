...
class SendSMSTransportInfosmska extends SendSMSTransport {
	...
	function __construct() {
		global $CONFIG;
		if (!isset($CONFIG['sendsms']))
			throw new Exception("Работа с sms не настроена!");
		if (!@$CONFIG['sendsms']['login'])
			throw new Exception("Работа с sms не настроена (не указан логин)");
		if (!@$CONFIG['sendsms']['password'])
			throw new Exception("Работа с sms не настроена (не указан пароль)");
		$postdata = array(
		...
		if ($balance < 0.5)
			throw new Exception("Не достаточно средств ($balance) на счете для отправки sms!");
	}
	...
	function setNumber($number) {
		if (preg_match('/^\+7\d{1,15}$/', $number))
			...
			throw new Exception('Номер для SMS указан в недопустимом формате!');
		$this->number = $number;
	}
	...
	function send() {
		global $CONFIG;
		if (!$this->number)
			throw new Exception("Номер отправки не указан" . $this->number);
		if ($this->text == '')
			throw new Exception("Текст собщения не задан");
		...
		if ($output[0] != 'o' && $output[0] != 'O')
			throw new Exception("Ошибка передачи сообщения: " . $output);
	}
...
class SendSMSTransportVirtualofficetools extends SendSMSTransport {
	...
	function send() {
		...
		if ($errnode->getAttribute('code') != 0)
			throw new Exception("Ошибка от сервиса передачи: " . $errnode->getAttribute('message'));
	}
}
