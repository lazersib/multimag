...
function MakeContextMenu(txt)
{
	...
	if((mousX+300)>windowWidth)	popup.style.left=windowWidth-300+scrollLeft;
	else popup.style.left=mousX-5+scrollLeft;
	if((mousY+150)>windowHeight)	popup.style.top=windowHeight-150+scrollTop;
	else popup.style.top=mousY-5+scrollTop;
	
	popup.id="pup"+(Math.floor(Math.random()*1000));
	popup.innerHTML=txt;
	popup.onmouseout=function() { ContextMenuOut(popup); };
	popup.onmouseover=function() { ContextMouseIn(); };
	return popup;
}

function ContextMouseIn()
{
	if(context_timer) window.clearTimeout(context_timer);
}

function ContextMenuOut(popup)
{
	if(context_timer) window.clearTimeout(context_timer);
	popup_=popup;
	context_timer=window.setTimeout("HideContextMenu(popup_)", 250);
}

function HideContextMenu(popup)
{
	if(popup)
	{
		var body = document.getElementsByTagName('body');
...
function ShowContextMenu(url)
{
	...
	if (!httpRequest) { return false; }	
	var popup=MakeContextMenu("<img src='img/icon_load.gif'> Загрузка...");	
	httpRequest.onreadystatechange = function() { ShowContextMenuReq(httpRequest,popup); };
	httpRequest.open('GET', url, true);
	httpRequest.send(null);
}

function ShowContextMenuReq(httpRequest,popup)
{
	if (httpRequest.readyState == 4)
	{
		if (httpRequest.status == 200)	popup.innerHTML = httpRequest.responseText;
		else 				popup.innerHTML="Ошибка "+httpRequest.status;
	}
	else if (httpRequest.readyState == 2)
		popup.innerHTML="<img src='img/icon_load.gif'> Загрузка...";
