...
if ($opt == 'jabber') {
    if(!is_array(@$CONFIG['site']['allow_jids'])) {
        $to = $CONFIG['site']['doc_adm_jid'];
    }
    elseif(!in_array($to, $CONFIG['site']['allow_jids'])) {
        $to = $CONFIG['site']['doc_adm_jid'];
    }
} else {
    if(!is_array(@$CONFIG['site']['allow_emails'])) {
        $to = $CONFIG['site']['doc_adm_email'];
    }
    elseif(!in_array($to, $CONFIG['site']['allow_emails'])) {
        $to = $CONFIG['site']['doc_adm_email'];
    }
}
...
if ($mode == "") {
    ...
    if ($opt == 'email') {
        $desc = 'при помощи эелектронной почты (e-mail)';
    }
    ...
    $text = request('text');
    $text = "Нам написал сообщение $nm($backadr)с сайта {$pref->site_name}\n-------------------\n$text\n";
    ...
    if ($opt == 'jabber') {
        ...
        try {
            mailto($to, "Сообщение с сайта {$pref->site_name}", $text);
            $tmpl->msg("Сообщение было отправлено!", "ok");
        } catch (Exception $e) {
            writeLogException($e);
        ...
        if ($name && $phone && $call_date) {
            if (@$CONFIG['call_request']['captcha'] && (@$_REQUEST['img'] == '' ||
                ...
                try {
                    ...
                    $text = "Посетитель сайта {$pref->site_name} $name просит перезвонить на $phone в $call_date";

                    if (@$CONFIG['call_request']['email']) {
                        mailto($CONFIG['call_request']['email'], "Запрос звонка с сайта {$pref->site_name}", $text);
                    }
            ...
            $tmpl->msg("Не заполнено одно из полей!", "err");
        }
    }
    if (!$ok) {
...
elseif ($mode == 'petitions') {
    ...
    if (mb_strlen($comment) > 8) {
        ...
        if ($user_info['worker_email'] != '') {
            ...
            $from = $CONFIG['site']['doc_adm_email'];
        }
        ...
        $tmpl->msg("Опишите причину подробнее!");
    }
}
