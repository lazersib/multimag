...
if($rights['write'])
{
	$tmpl->SetText("<h1>Настройка прав</h1>");
	$tmpl->SetTitle("Настройка прав");
	if($mode=='')
	{
		$tmpl->AddText("<h3>Группы пользователей</h3><table><tr><th>N<th>Название<th>Описание");
	...
	else if($mode=='group')
	{
		...
		<table><tr><th>Объект<th>Чтение<th>Запись<th>Изменение<th>Удаление");
		$res=mysql_query("SELECT `id`, `object`, `a_read`, `a_write`, `a_edit`, `a_delete`
		FROM `users_grouprights`
		WHERE `gid`='$g'");
		while($nxt=mysql_fetch_row($res))
		{
			$tmpl->AddText("<tr><td><a href='?mode=obje&amp;o=$nxt[1]&amp;g=$g'>$nxt[1]<img src='/img/i_edit.png' alt='Изменить'></a>
			<td>$nxt[2]<td>$nxt[3]<td>$nxt[4]<td>$nxt[5]");
		}
